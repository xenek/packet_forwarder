...
# 
# Written by Jac Kersing <j.kersing@the-box.com>, with changes by Eric Gourlaouen <eric@thethingsindustries.com>
#
# Parts of the script based on tzselect by Paul Eggert.
#

STATUSFILE=/var/config/.installer
# STATUSFILE allows us to keep track of the installation process.
...
if [ $? -ne 0 ] ; then
	...
	echo "$0: Device already secured, ignoring"
fi

grep timezone $STATUSFILE > /dev/null 2> /dev/null
if [ $? -ne 0 ] ; then
	echo "$0: Starting timezone configuration"
	# -------------------------------- from tzselect code: --------------------
...
'
	...
	echo "$0: Device timezone already configured, ignoring"
fi
...
_EOF_
	...
	echo "power."
	echo ""
	echo "When the gateway reboots, continue the configuration process with"
	echo "the same /etc/init.d/ttn-pkt-fwd configure command."
	echo ""
	...
	echo "$0: Device network already configured, ignoring"
fi
...
_EOF_
	...
	echo "$0: Multitech LoRa server already disabled, ignoring"
fi

# Set date and time using ntpdate
grep date $STATUSFILE > /dev/null 2> /dev/null
if [ $? -ne 0 ] ; then
	echo "$0: Date/time configuration"
	...
	if [ $? -ne 0 ] ; then
		...
		echo "Network settings are necessary to run the last step of the setup"
		echo "(update date and time info using ntpdate)."
	...
	echo "$0: Date and time already configured, ignorin"
fi
